...
3. From what you actually read, **enumerate every case as a checkbox** under that block:
   ...
   - **every authorization gate** — if the unit reads a role, permission, or ownership / scope check (renders, enables, or hides on it, or calls a gated action), one permission state is not enough: it needs a **permission matrix** (Step 5.2), with at least one permission state that is *refused*.
...
### Step 5.2 — Permission-gated units: the permission matrix

When a unit's behaviour depends on the **permission state** — a role, a permission, a gate, an ownership / tenant / scope check — one happy-path permission state is not a test. This is the single biggest source of escaped bugs, because a harness that always runs under the same permission state (or only ever the granted one) never exercises the refused paths. Apply this whenever Step 5.0 flagged an authorization gate.

**Drive the permission state directly.** The permission is an **input** to the unit — the gate boolean the code reads (e.g. `gates.authorized_to_*`), the user's permission list, a role / scope flag — scoped as the target project's model requires (global, per-tenant / organisation, or per-resource-owner). In unit / integration you **supply or mock that input directly**, one value per case; you do not need a real user or a persona catalogue behind it, and you must not fall back to a single default state. In E2E against a real auth system you can drive the same states through real users instead. What matters is that both the unit / integration tests and the E2E suite exercise the **same permission states** through the same scenarios — so when the two layers disagree on what a given state allows, that divergence is the bug.

**Build a matrix, not a list.** For each **scenario** the unit exposes × each **permission state**, state the expected outcome. The permission is the *input* that sets the expected result; you assert the **observable outcome** — element present / absent, action succeeds / is refused — never "does this user hold permission Y" (that just re-tests the framework's gate).
...
- **Weight the matrix on the refused cells.** "This state is allowed" is the easy half; the bugs live in "this state must **not**, and is actually prevented." For every gated capability, test at least one permission state that is denied.
...
- **Aggregation across several roles needs unit coverage.** An E2E harness that only exercises single-role states will never surface a bug in how rights *combine* across roles. Add a unit test built on a permission state where the capability rides a **non-primary** role, so an aggregation regression fails at the unit level.

**Prerequisite — you must be able to drive the permission state.** This chapter needs a way to set the gate / permission the unit reads. In unit / integration that is just feeding the input (the gate boolean, the permission list); in E2E it may mean real test users. If you **cannot drive the permission state at all**, say so — do not write a green test locked to one default state, and do not silently fall back to it. Otherwise, test across the permission states you can drive: a single permission state is not a test, and a matrix run under one state is the very hole this step closes.
