...
- **Don't assume `@types/<package>` is needed just because older ecosystem habits say so.** Some packages have transitioned from relying on DefinitelyTyped to shipping their own bundled types (e.g. `@hapi/hapi` from v21 on) — a separately-installed `@types/*` stub can be stale relative to the installed package version and mask real type errors. Check the package's own `package.json` `types`/`typings` field, or just try removing the `@types/*` package, before assuming it's required.
