...
| Fastify | `fastify` in `dependencies` | No built-in auth (same as Express); extend `FastifyRequest` via `decorateRequest` + `declare module 'fastify'` for a typed `request.user`. Fastify's default error handler reads a thrown `Error`'s `.statusCode` property automatically — a route handler doesn't need an explicit `reply.code(...)` call on every error path, unlike every other framework in this doctrine. Test with `app.inject()` (in-process, no real socket) — but see Step 4's fake-timers note, it is **not** immune to the real-socket fake-timer hazard despite not opening one |
| Hapi | `@hapi/hapi` in `dependencies` | Auth is a first-class abstraction (`server.auth.scheme(...)`/`server.auth.strategy(...)`/`server.auth.default(...)`), not a hook/middleware — read the actual scheme rather than assuming a Fastify-style hook. Use `@hapi/boom` errors (`Boom.unauthorized()`/`.forbidden()`/`.notFound()`), thrown directly, translated automatically. Validate via Joi as a route option (`options.validate.payload`), Hapi's own idiomatic default — not Zod. `@hapi/hapi` ships its own bundled types as of v21; don't install `@types/hapi__hapi`, it may be stale relative to the installed version. `request.headers.authorization` types as `{}` under `strict: true` — cast to `string \| undefined`. Test with `server.inject()` (in-process, no real socket, payload is a raw string requiring `JSON.parse`) — same fake-timers hazard as Fastify's `app.inject()`, see Step 4 |
| Koa | `koa` in `dependencies` | No built-in router/body-parser/auth (leanest core of any framework here) — `@koa/router` and `koa-bodyparser` are separate packages, auth is hand-rolled middleware. Errors thrown via `ctx.throw(status, message)` (typed `never`), but this only narrows control flow if the enclosing middleware/handler's `ctx` parameter is **explicitly typed** — an inferred parameter silently breaks the narrowing even though the type is structurally the same. `@koa/router` ships its own bundled types; `koa` itself does not (still needs `@types/koa`) — check each package independently. No native in-process test injector — test via `supertest` against `app.callback()`, and see Step 4's fake-timers note since this is a real-socket client like tRPC/GraphQL |
...
- **Don't assume `@types/<package>` is needed just because older ecosystem habits say so.** Some packages have transitioned from relying on DefinitelyTyped to shipping their own bundled types (e.g. `@hapi/hapi` from v21 on, `@koa/router`) — a separately-installed `@types/*` stub can be stale relative to the installed package version and mask real type errors. Check the package's own `package.json` `types`/`typings` field, or just try removing the `@types/*` package, before assuming it's required. **Check per-package, not per-ecosystem**: Koa's own core package (`koa`) still has no bundled types and genuinely needs `@types/koa`, even though its own official router package (`@koa/router`) has already made the switch.
- **A `never`-typed guard function (`ctx.throw()`, an `assert()` helper, a Boom-style throw) only narrows control flow reliably when the enclosing function's parameters are explicitly typed.** Found in the Koa example: `ctx.throw(401, '...')` is typed to return `never` in `@types/koa`, and TypeScript's control-flow analysis is documented to treat code after an unconditional call to a `never`-returning function as unreachable — but this silently stopped working when the middleware's `ctx` parameter was left to be inferred from `app.use(...)`'s own overloaded signature, even though the inferred type is structurally identical to an explicitly-annotated one. Fix: annotate the parameter explicitly (`(ctx: Koa.Context, next) => {...}`, or a project-specific alias). If a `never`-typed guard isn't narrowing as expected, try this before reaching for non-null assertions.
...
- **A test whose HTTP-driving mechanism's async plumbing depends on real timer functions must not blanket-fake Jest's timers — and this is broader than "opens a real socket."** `jest.useFakeTimers()` without `doNotFake` replaces `setTimeout`/`setImmediate`/etc. globally. This obviously breaks a test that opens a real socket (`http.createServer(...).listen(0)` driven via `fetch`/a real client) since Node's HTTP/net stack schedules socket I/O through those functions — but it **also** breaks Fastify's own `app.inject()` and Hapi's own `server.inject()`, neither of which ever opens a socket, because both frameworks' internal request-lifecycle machinery (`avvio` boot sequencing for Fastify, Hapi's own dispatch sequencing) itself depends on the same global timer functions. `supertest`-driven tests (Express, Adonis via Japa) are not affected. Pass `doNotFake: ['setTimeout', 'clearTimeout', 'setInterval', 'clearInterval', 'setImmediate', 'clearImmediate', 'nextTick', 'hrtime', 'performance', 'queueMicrotask']` to freeze only `Date`/`Date.now()` while leaving the timer/async machinery real. **If a scheduling/isolation test hangs under `jest.useFakeTimers()` regardless of which HTTP-driving mechanism is in use, try this fix first** — confirmed now across a real-socket client (tRPC, GraphQL, Koa via `supertest`) and two different frameworks' own native in-process injection helpers (Fastify, Hapi); don't assume a framework's own "fast, in-process" test helper is automatically safe just because it avoids a real socket, and don't assume a real-socket-driven test is somehow "safer" than an in-process one either — both need the same fix.
