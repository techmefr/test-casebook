...
3. **Add the `data-test-*` hooks** each case needs directly to the markup/JSX (or Blade view) — never reach for CSS, tag structure, or visible text to cover an awkward case.
4. **Type everything strictly** (Pass B step 3): real prop/fixture types, typed mocks/spies to the signature they replace, narrow DOM types, no `any`, no blind `as`. The file must pass the project's type-check / lint with zero new errors. **PHP/Pest:** `declare(strict_types=1)`, typed fixtures/factories against real Models/Resources/DTOs, no untyped array shape standing in for a domain object, and the file must pass `vendor/bin/phpstan analyse` (Larastan, level 8) with zero new errors — no baseline-ignore — see `docs/testing-guide/laravel.md`.
5. **Isolate and stay deterministic** (Step 5.1): mock the network (MSW, or `Http::fake()` on Laravel) with fixtures typed from the real contract, seed stores (`RefreshDatabase` + factories on Laravel), provide router/i18n via test utils, freeze time (`vi.useFakeTimers()`, or `Carbon::setTestNow()` on Laravel), pin timezone/locale. A mock that has drifted from the real shape is worse than no mock.
