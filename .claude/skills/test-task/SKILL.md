...
2. **Interview the PM to close gaps — one question at a time**, highest-value ambiguity first (never a wall of questions):
   - which **permission states** are in play — each a role / permission set + scope (global / per-tenant-or-BU / resource-owner);
...
4. **Build one internal matrix** `permission state × scenario → expected observable outcome` (allowed / refused), then **split it into one sub-task per team**. Deriving all sub-tasks from the *same* matrix is what keeps front and back from drifting on what a permission state is or what's expected.
5. **Create the Jira sub-tasks** (issue type **Sub-task** unless the project uses a custom one — confirm against the project's issue types), each **linked to the parent story**:
   ...
   - each sub-task **self-contained**: it restates the permission states and the expected outcomes for *its* layer, so the developer needn't read the sibling sub-tasks.
...
- **Dense on the refused cells** — for every gated capability, at least one permission state that must be **denied**.
...
- **Both layers must agree** — the same permission state / scenario appears in the front *and* back sub-task with matching intent; a front/back disagreement at execution time is the bug this is meant to surface.
...
The refused/allowed cells are only real if each developer can **drive the permission state** in the test (feed the gate / permission as an input; in E2E, real test users). If a team cannot drive it at all, say so in its sub-task so it's addressed before the cells are trusted — see the permission matrix in `AGENTS.md` (Step 5.2).
